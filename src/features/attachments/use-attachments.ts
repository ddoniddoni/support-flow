...
import { createAttachmentName, nextAttachmentSequence } from "./file-name";
import {
  attachmentAccept,
  attachmentMaxBytes,
  attachmentMaxCount,
...
type Item = Pick<Attachment, "id" | "name" | "size" | "mime_type"> & {
...
function createQueue(scope: string, ticketId?: string, internal = false) {
  ...
  function publish(next: Snapshot) {
    ...
    try {
      sessionStorage.setItem(
        ...
        JSON.stringify(
          next.items
            ...
            .map(({ id, name, size, mime_type, status, sequence }) => ({
              id,
              name,
              size,
              mime_type,
      ...
      /* Existing uploads remain usable in this page. */
    }
  ...
  async function upload(item: Item) {
    ...
    try {
      ...
      form.set("sequence", String(item.sequence ?? 1));
      form.set("internal", String(internal));
      ...
      publish({
        ...
        items: snapshot.items.map((current) =>
          current.id === item.id
            ? { ...current, ...body.attachment, status: "ready" }
            : current,
        ),
      });
    } catch (error) {
  ...
  return {
                ...
                (item): item is Item =>
                  ...
                  (item.sequence === undefined ||
                    (Number.isSafeInteger(item.sequence) &&
                      item.sequence >= 1 &&
                      item.sequence <= 10000)) &&
                  attachmentAccept.split(",").includes(item.mime_type),
              )
              .slice(0, attachmentMaxCount),
            error: null,
          });
      } catch {
        /* Ignore corrupt saved selections. */
      }
    },
    async add(files: File[]) {
      if (snapshot.items.length + files.length > attachmentMaxCount) {
        publish({
          ...snapshot,
          error: "첨부파일은 최대 5개까지 선택할 수 있습니다.",
        });
        return;
      }
      if (
        files.some(
          (file) =>
            ...
            !attachmentAccept.split(",").includes(file.type),
        )
      ) {
        publish({
          ...snapshot,
          error: "JPG·PNG·WebP·PDF 파일을 3MB 이하로 선택해 주세요.",
        });
        return;
      }
      const firstSequence = nextAttachmentSequence(snapshot.items);
      if (firstSequence + files.length - 1 > 10000) {
        publish({
          ...snapshot,
          error: "첨부파일 번호를 다시 시작하려면 현재 첨부를 제거해 주세요.",
        });
        return;
      }
      const items: Item[] = files.map((file, index) => ({
        id: crypto.randomUUID(),
        name: createAttachmentName(firstSequence + index, file.type, file.name),
    ...
    getIds: () => {
      if (snapshot.items.some((item) => item.status !== "ready"))
        throw new Error(
          "첨부파일 업로드를 완료하거나 실패한 파일을 제거해 주세요.",
        );
...
export function useAttachments({
  ...
  return {
    ...snapshot,
    queue,
    blocked: snapshot.items.some((item) => item.status !== "ready"),
  };
}
