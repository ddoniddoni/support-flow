...
export function CustomerHeader({ name, profileId }: { name: string; profileId: string }) {
  ...
  return (
    <header className="border-b border-border bg-card">
      <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-x-6 gap-y-4 px-5 py-4 sm:px-8">
        ...
        <div className="flex items-center gap-2"><span className="mr-2 hidden max-w-40 truncate text-sm text-muted-foreground sm:block">{name}님</span><NotificationBell profileId={profileId} compact /><ThemeToggle /><LogoutButton /></div>
