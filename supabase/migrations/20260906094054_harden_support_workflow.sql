-- Apply after docs/SUPABASE_SCHEMA.sql and docs/AI_SCHEMA_MIGRATION.sql.
-- Existing data is preserved. All mutations and their audit records commit together.
begin;
...
begin
  ...
  values (new.id, new.email,
    ...
    'customer') on conflict (id) do nothing;
  return new;
end;
$$;
...
alter table public.tickets add constraint tickets_input_lengths
  check (length(btrim(title)) between 5 and 120 and length(btrim(content)) between 20 and 4000) not valid;
alter table public.ticket_replies add constraint replies_content_length
  check (length(btrim(content)) between 3 and 4000) not valid;

-- RLS restricts rows, not columns. AI fields on tickets must also be protected.
revoke all on public.tickets from anon;
revoke select, insert, update, delete on public.tickets from authenticated;
grant select (id, title, content, status, priority, category, ticket_number,
  customer_id, assignee_id, created_at, updated_at) on public.tickets to authenticated;
grant insert (title, content, category, customer_id) on public.tickets to authenticated;
...
for insert to authenticated with check (
  auth.uid() = customer_id and public.current_user_role() = 'customer'
);

revoke all on public.ticket_replies, public.ticket_logs, public.ticket_ai_analyses,
  public.ticket_ai_review_events, public.ai_prompt_versions from anon;
revoke insert, update, delete on public.ticket_replies, public.ticket_logs,
  public.ticket_ai_analyses, public.ticket_ai_review_events from authenticated;
-- Retire the old endpoint so callers cannot bypass the new transaction rules.
revoke all on function public.create_ticket_reply(uuid, text, boolean) from public, anon, authenticated;
revoke all on function public.current_user_role() from public, anon;
revoke all on function public.can_view_profile(uuid) from public, anon;
grant execute on function public.current_user_role(), public.can_view_profile(uuid) to authenticated, service_role;

alter table public.ticket_ai_analyses
  add column review_decision public.ai_review_decision;
create index ticket_ai_latest_idx on public.ticket_ai_analyses(ticket_id, created_at desc, id desc);

-- The lateral query uses the caller's AI RLS: a customer receives no AI values.
create view public.ticket_workspace with (security_invoker = true) as
select t.id, t.title, t.content, t.status, t.priority, t.category, t.ticket_number,
  ...
  case when a.id is null then null else to_jsonb(a) - 'raw_response' end as ai_analysis
...
revoke all on public.ticket_workspace from public, anon;
grant select on public.ticket_workspace to authenticated, service_role;

-- Privileged implementation lives outside exposed schemas. Every command checks
-- the authenticated actor and locks the ticket before making business decisions.
create schema if not exists supportflow_private;
revoke all on schema supportflow_private from public, anon;
grant usage on schema supportflow_private to authenticated, service_role;
create function supportflow_private.ticket_command(p_ticket_id uuid, p_action text, p_payload jsonb)
...
begin
  ...
  if ticket.id is null or (actor_role = 'agent' and ticket.assignee_id is distinct from actor)
    or (actor_role = 'customer' and not (trusted_service and p_action = 'save_analysis'
      and ticket.customer_id = actor and ticket.latest_ai_analysis_id is null)) then
  ...
  if p_action = 'update' then
    ...
    -- Preserve the product's one public answer per ticket workflow.
    if not internal and exists(select 1 from public.ticket_replies where ticket_id=ticket.id and not is_internal) then
      raise exception '이미 고객 답변이 등록된 문의입니다.' using errcode = '23505';
    end if;
    insert into public.ticket_replies(ticket_id,author_id,content,is_internal)
    values(ticket.id,actor,btrim(p_payload->>'content'),internal) returning * into reply;
    insert into public.ticket_logs(ticket_id,actor_id,action,after_value)
    values(ticket.id,actor,case when internal then 'internal_note_added' else 'reply_added' end,reply.id::text);
    if not internal and source='ai_draft' then
      insert into public.ticket_logs(ticket_id,actor_id,action,after_value)
      values(ticket.id,actor,'ai_draft_used_as_customer_reply',reply.id::text);
    end if;
    if not internal and ticket.status in ('open','in_progress') then
      update public.tickets set status='resolved' where id=ticket.id;
      insert into public.ticket_logs(ticket_id,actor_id,action,before_value,after_value)
      values(ticket.id,actor,'status_changed',ticket.status::text,'resolved');
    end if;
...
$$;
revoke all on function supportflow_private.ticket_command(uuid,text,jsonb) from public, anon;
grant execute on function supportflow_private.ticket_command(uuid,text,jsonb) to authenticated,service_role;
create function public.support_ticket_command(p_ticket_id uuid,p_action text,p_payload jsonb default '{}')
returns jsonb language sql security invoker set search_path = '' as $$
  select supportflow_private.ticket_command(p_ticket_id,p_action,p_payload)
$$;
revoke all on function public.support_ticket_command(uuid,text,jsonb) from public,anon;
grant execute on function public.support_ticket_command(uuid,text,jsonb) to authenticated,service_role;
commit;
